...
- Updated RAM data for v2025
- Extended the date range for the RAM data from 2019 to 2023. Previous assessments had stopped at 2019 to match the SAUP data. This was NOT done for the 2025 assessment and data up to 2023 was used. 
...
3.  timeseries_values_views # checked v2025
...
Abbreviations:
- `TBdivTBmsy` = Total biomass relative to TBmsy
- `SSBdivSSBmsy` = Spawning stock biomass relative to SSBmsy
- `TBdivTBmgt` = Total biomass relative to TB management target
- `SSBdivSSBmgt` = Spawning stock biomass relative to SSB management target
- `stocklong` = Stock name

Definitions:
- Spawning stock: mature, breeding age fish
- Total biomass: all fish in the stock
...
RAM_dir <- ("/home/shares/ohi/OHI_GOC/_raw_data/ram/v2025/")

load(file.path(RAM_dir, "RAMLDB_v466_3/R Data/DBdata[asmt][v4.66].RData")) # update if downloading a new database

stocks <- metadata 

table(stocks$region)
filter(stocks, primary_country=="Mexico")
filter(stocks, primary_country=="USA") %>% arrange(scientificname) %>% data.frame()
filter(stocks, region=="Pacific Ocean") %>% arrange(scientificname)
filter(stocks, region=="US West Coast") %>% arrange(scientificname)
filter(stocks, region=="US West Coast (Pacific Salmon)") %>% arrange(scientificname)
filter(stocks, region=="South America") %>% arrange(scientificname)
filter(stocks, region=="Other" ) %>% arrange(scientificname)

filter(stocks, primary_FAOarea == 77 & !(FisheryType %in% "Rockfish"))

stocks_ids <- filter(stocks, stockid %in% c("THRSHARNPAC", "YFINEPAC", "SARDPCOAST", "CMACKPCOAST", "BIGEYEEPAC", "BLSHARNPAC",
                              "SKJEPAC", "STMARLINNEPAC", "STMARLINWCNPAC", "SWORDEPAC", "SFMAKONPAC", "SARDPCOAST"))

filter(timeseries_values_views, stockid == "SARDPCOAST" & year > 2000)
...
ram <- ram_bmsy_new %>%
  filter(stockid %in% stocks_ids$stockid) %>%
  select(stockid, year, ram_bmsy)


ram_current <- left_join(ram, stocks_ids, by="stockid")
write_csv(ram_current, here("FP/FIS/v2025/data/ram.csv"))
```
...
ram_bmsy_gf <- ram_bmsy_new %>%
  filter(year >= 2001 & year <= 2023) %>%   # v2025: changed year to 2023 from 2019. 2019 is the last year of SAUP data. 
  group_by(stockid) %>%
  mutate(max_year = max(year),
         # It doesn't seem like we actually use these columns. Not sure if still necessary
         years_data_2005_now = length(ram_bmsy[year >= 2005]),
         years_data_2001_now = length(ram_bmsy[year >= 2001])) %>%
  ungroup() %>%
...
year_gap_stocks <- ram_bmsy_gf %>%
  filter(diff_years > 1)

# view year gap stocks
year_gap_stocks_id <- unique(year_gap_stocks$stockid)
year_gap_stocks_id # v2025: [1] "SDOGATLC" "WPOLLWBS"

# remove diff_years for gapfilling
ram_bmsy_gf2 <- ram_bmsy_gf %>%
  dplyr::select(-diff_years)


## Get rows for stocks/years with no B/Bmsy (identified as NA B/Bmsy value for now)
ram_bmsy_gf_3 <- ram_bmsy_gf2 %>%
  spread(year, ram_bmsy) %>% 
...
# remove stocks with year gaps
ram_bmsy_gf_4 <- ram_bmsy_gf_3 %>%
  filter(!stockid %in% c(year_gap_stocks_id))

# for those that don't have year gaps, gapfill by drawing the recent values up or down
...
# df of stocks that have year gaps
ram_bmsy_gf_year_gap <- ram_bmsy_gf_3 %>%
  filter(stockid %in% c(year_gap_stocks_id)) %>%
  group_by(stockid) %>%
...
stocks <- unique(ram_bmsy_gf_year_gap$stockid) # v2025: [1] "SDOGATLC" "WPOLLWBS" 
stocks
...
for(stock in stocks){
  ...
  filter_stock <- ram_bmsy_gf_year_gap %>%
...
ram_bmsy_gf_interpolate_2 <- ram_bmsy_gf_interpolate %>%
  ...
  mutate(gapfilled = case_when(
    stockid == "SDOGATLC" & year %in% c(2023) ~ "down/upfilling",
    TRUE ~ gapfilled
  )) %>%
...
table(tmp$gapfilled) # only 2 stocks use linear interpolation for some years. 418 are down/upfilled. 434 stocks are not gapfilled at all

sum(table(tmp$gapfilled))  
# 854 stocks have at least one year of bbmsy values after 2010

# check that everything has been taken care of
colSums(is.na(ram_bmsy_gf_final)) # no NAs for the gf column
sum(ram_bmsy_gf_final$ram_bmsy_gf < 0 )  # should be 0. 2025: 0
min(ram_bmsy_gf_final$ram_bmsy_gf, na.rm = TRUE) # v2025: 0.002531646
...
# Check the number of unique stocks over time
stocks_year <- ram_bmsy_gf_final_fin %>%
  filter(year >= 2001) %>% 
  distinct(year, stockid) %>%
  count(year, name = "num_stocks")

ggplot(stocks_year, aes(x = year, y = num_stocks)) +
  geom_point() +
  labs(x = "Year",
       y = "Number of stocks",
       title = "Unique stocks over time") +
  theme_minimal()
```
...
#               Estimate
# (Intercept) -3.254e-16
# ram_bmsy_gf  1.000e+00
#             Std. Error
# (Intercept)  1.621e-18
# ram_bmsy_gf  9.500e-19
#                t value Pr(>|t|)
# (Intercept) -2.008e+02   <2e-16
# ram_bmsy_gf  1.053e+18   <2e-16
#                
# (Intercept) ***
# ram_bmsy_gf ***
# ---
...
#   0.05 ‘.’ 0.1 ‘ ’ 1
# 
# Residual standard error: 9.08e-17 on 7628 degrees of freedom
#   (2352 observations deleted due to missingness)
# Multiple R-squared:      1,	Adjusted R-squared:      1 
# F-statistic: 1.108e+36 on 1 and 7628 DF,  p-value: < 2.2e-16
# 
# Warning message:
# In summary.lm(mod) : essentially perfect fit: summary may be unreliable
```
...
**Issue 2025**: I cannot find this intermediate file. It looks like it gets made in the next step, so jumping there via the note above. Not sure why these are out of order...
...
# v2025:
#  [1] "TARAKNZ"       "HERR2529-33"   "HERR2529-34"  
#  [4] "HERR2529-35"   "HERR2529-36"   "HERR2529-37"  
#  [7] "HERR30-32"     "SOLEIIIa-2225" "SOLEIIIa-2226"
# [10] "SOLEIIIa-2227" "SOLEIIIa-2228"
...
setdiff(ram_spatial_meta$stockid, ram_meta$stockid) # v2025: now it is 0, we can move on
```
...
tmp <- data.frame(scientificname = sort(setdiff(ram_sp$scientificname, SAUP_sp$SAUP_scientificname))) 
tmp # 40 species names

# compare names - what's in SAUP that's not in RAM
tmp2 <- data.frame(scientificname = sort(setdiff(SAUP_sp$SAUP_scientificname, ram_sp$scientificname))) 
nrow(tmp2) # 2341 species names.. unfortunately a lot.
...
SAUP_sp_fao_ohi <- read_csv(here(dir_M, "git-annex","globalprep","fis","v2022","int", "stock_catch_by_rgn_taxa.csv")) %>% # use most recent one available. v2025: still no new data here
  dplyr::rename(SAUP_scientificname = TaxonName) %>%
...
# v2025: [1] "Mullus barbatus": could not find 


# v2025 adding new species (change to relevant species for current year):
...
**Issue 2025**: For line 507, not super sure what it means about hand-checking and adding the missing species to the RAM_species_to_SAUP.csv. The setdiff() in the line below seems to indicate that there is only one species that does not match between them?
...
ram_name_corr # matched species, unfortunately only 14
...
length(unique(ram_spatial$stockid)) # v2025: 501 RAM stocks with B/Bmsy data
length(unique(ram_spatial$species)) # v2025: 238
```
...
# v2023 and v2024 and v2025: for some reason SFMAKOSATL and HERR4RFA are not in ram_spatial. SFMAKOSATL was determined to be because the fishing/OHI region, the US, is not in the FAOs of the South Atlantic, so this one is ok. HERR4RFA seemed to also show up in v2023 and v2021, so it is uncertain why this one was not in ram_spatial, and it will be manually added in. Could not find RAM_area_m2 info for it so putting NA, which is consistent with HERR4RSP.
# stockid: HERR4RFA
...
# [58] "BLINGVb-VI-VII"              
# [59] "HADNS-IIIa-VIa"              
# [60] "HMACKIIa-IVa-Vb-VIa-VII-VIII"
# [61] "HMACKIXa"                    
# [62] "NEPHFU14"                    
# [63] "NEPHFU17"                    
# [64] "NEPHFU7"                     
...
```{r}
# Last year's data
ram_2024 <- read_csv(here("globalprep", "fis", "v2024","int","ram_bmsy.csv")) #%>%
  # dplyr::mutate(stockid = substr(stockid, 1, nchar(stockid) - 3)) %>%
  # drop_na(ram_bmsy)

# This year's data
ram_2025 <- read_csv(here(int_dir, "ram_bmsy.csv"))

compare <- ram_2025 %>%
  left_join(ram_2024, by = c("rgn_id", "stockid", "stockid_ram", "stocklong", "year", "RAM_area_m2")) %>%
...
comparison_plot <- ggplot(compare, aes(x = ram_bmsy.y, y = ram_bmsy.x)) +
  ...
  labs(
    title = "RAM data B/Bmsy Values: v2024 vs v2025",
    x = "v2024 B/Bmsy",
    y = "v2025 B/Bmsy"
  ) +
  theme_minimal() +
...
largest_diffs_plot <- ggplot(compare, aes(x = ram_bmsy.y, y = ram_bmsy.x)) +
  ...
  theme_minimal() +
  labs(title = "v2024 Bmsy data vs. v2025 Bmsy data", x = "v2024 BMSY", y = "v2025 BMSY")
largest_diffs_plot
```

When it matches that well, making sure I'm actually pulling from the right files and such. Quick check that. theoretically, don't think it's a huge issue. One good through check, if it looks good we'll assume it's correct. Didn't modify any of the 2019 data etc. 
...
combined_plot <- y_plot + x_plot +
  plot_layout(ncol = 2) +
  plot_annotation(
    title = "RAM Bmsy Distributions: v2024 vs v2025"
  )
combined_plot # histogram distributions look pretty similar
```

```{r}
# v2025 number of stocks over time plot
nrow(ram_2025)

ram_2025_stocks <- ram_2025 %>%
  distinct(year, stockid_ram, rgn_id) %>%
  count(year, name = "num_stocks") %>%
  view()
# 3167 for all years... hmm

# There are the same number of stocks for each year. Need to go back and see if this is a gapfilling thing or a mistake somewhere along the workflow. 
```
