...
filter(stocks, primary_FAOarea == 77)

# stocks: THRSHARNPAC, YFINEPAC, BIGEYEEPAC,  BLSHARNPAC, SFMAKONWPAC
#
...
sum(test$num_max_yrs) # 476 total stocks

plot(test)

# Check the number of unique stocks over time
stocks_year <- ram_bmsy_new %>%
  filter(year >= 2001) %>% 
  distinct(year, stockid) %>%
  count(year, name = "num_stocks")

stocks_year_only <- ram_bmsy_new %>%
  filter(year >= 2001) %>% 
  count(year, name = "num_stocks")
...
stocks_year_only <- ram_bmsy_new %>%
  ...
  count(year, name = "num_stocks") %>% 
  arrange(year) %>% 
  mutate(diff = num_stocks - lag(num_stocks))

ram_2025_stocks <- ram_bmsy_new %>%
  filter(year >= 2001) %>% 
  distinct(year, stockid) %>%
  count(year, name = "num_stocks") %>%
  arrange(year) %>% 
...
ggplot(stocks_year_only, aes(x = year, y = diff)) +
  geom_point() +
  labs(x = "Year",
       y = "Number of stocks",
...
ggplot(ram_2025_stocks, aes(x = year, y = diff)) +
  geom_point() +
  labs(x = "Year",
       y = "Change in stock",
       title = "Change in unique stocks over time") +
  theme_minimal()
