...
## Necessary to generate clustered list for "cl"
if (Sys.getenv("RSTUDIO") == "1" && !nzchar(Sys.getenv("RSTUDIO_TERM")) && 
    Sys.info()["sysname"] == "Darwin" && getRversion() >= "4.0.0") {
  parallel:::setDefaultClusterOptions(setup_strategy = "sequential")
}
## See more info here: https://stackoverflow.com/questions/62730783/error-in-makepsockclusternames-spec-cluster-setup-failed-3-of-3-work
...
if(write.output==T){
  outheaders = data.frame("Group","Region", "Subregion","Name","SciName","Stock","start.yr","end.yr","btype",
             ...
             # ERICA COME BACK TO THESE LINES, 2001-2018             "c01","c02","c03","c04","c05","c06","c07","c08","c09","c10","c11","c12","c13","c14","c15","c16"...
  ...
  write.table(outheaders,file=outfile, append = T, sep=",",row.names=F,col.names=T)
}
...
#colnames(cdat)[5]<- "yr"
#colnames(cdat)[6]<- "ct"
colnames(cdat)<- c("Stock", "yr", "ct", "nsample")
cdat$ct = cdat$ct/1000 # Transform catch to tonnes

cinfo        <- read.csv(id_file, header=T, dec=".", stringsAsFactors = FALSE)
# For now, filter out the unsable stocks (-EMF)
cinfo <- cinfo[which(cinfo$Pick== 1),]
...
for (i in 1:nrow(cinfo)){ #nrow(cinfo)){#1:nrow(cinfo) or c(50,55,64,78,89,101)) {
  ...
  ct.raw2<- approx   (x = yr, y = ct.raw, xout = 2001:2019, method = "linear", n = 16,
            rule = 2, f = 0, ties = mean, na.rm = TRUE)
  ct.raw<- ct.raw2$y
  
  yr<- c(2001:2019) ## NOTE TO SELF: I wonder about this...
  ...
  ## I also wonder about this ^^ (above) because it doesn't actually seem to be plotting the correct max and min...?
  ...
  if (save.plots==TRUE) 
  {
    jpgfile<-paste("Output/",stock,".jpeg",sep="")
  ...
  {
    jpgfile<-paste("Output/",stock,"_MAN.jpeg",sep="")
    dev.copy(jpeg,jpgfile,
             width = 1024, 
             height = 768, 
             units = "px", 
