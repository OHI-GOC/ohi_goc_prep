---
title: 'Script 1 — Download Copernicus Chl-a (for NPP / climate pressure)'
...
This is **Script 1** for the climate / declining-productivity pressure work in
`_pressures/prs_npp/`.

It downloads Copernicus GlobColour **chlorophyll-a** and writes monthly GeoTIFFs.
**Script 2** (`script2_chla_regional_monthly_explore.Rmd`) explores those rasters.

**History:** Originally lived under Clean Waters eutrophication
(`CW/v2025/eutrophication/`). Chl-a was **not** retained as a eutrophication
indicator (see `CW/v2025/eutrophication/chla_not_used/chla_exploration_stepwise.Rmd`).
We keep the download here because the same monthly stack is useful for a
**different** pressure: climate-related declines in primary productivity
(using Chl-a as an interim proxy for NPP).

**Where the monthly rasters live today (already downloaded):**

`/home/shares/ohi/OHI_GOC/goal_prep/cw/v2025/chla_int/monthly_rasters/`

**Where new downloads / NPP intermediates should go:**

`/home/shares/ohi/OHI_GOC/goal_prep/pressures/npp/`

You do **not** need to re-run this script unless you want to refresh the time
series. Script 2 can read the existing `chla_int/monthly_rasters` files.
...
# NPP pressure intermediates on Mazu (preferred home going forward)
npp_dir <- "/home/shares/ohi/OHI_GOC/goal_prep/pressures/npp"

# Legacy location of the already-downloaded monthly stack (Script 2 can use this)
legacy_monthly_dir <- "/home/shares/ohi/OHI_GOC/goal_prep/cw/v2025/chla_int/monthly_rasters"

# Where THIS script writes monthly GeoTIFFs if you re-download
int_monthly_dir <- file.path(npp_dir, "monthly_rasters")
dir.create(int_monthly_dir, showWarnings = FALSE, recursive = TRUE)

# set colors
cols = rev(colorRampPalette(brewer.pal(9, 'Spectral'))(255)) # rainbow color scheme

message("New monthly rasters will be written to: ", int_monthly_dir)
message("Existing stack (if not re-downloading): ", legacy_monthly_dir)
```
...
# If already logged in on this machine, this may reuse the saved credentials file.
copernicusmarine.login()
# Example prior run: credentials stored under ~/.copernicusmarine/
