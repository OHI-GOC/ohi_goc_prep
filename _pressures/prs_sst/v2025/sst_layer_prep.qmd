...
terraOptions(threads = 50, progress = 3, todisk = TRUE, memfrac = 0.5)

process_sst_daily <- function(year, goc_ecoregion_border_shp) {
  ...
  daily_rasters <- lapply(files, function(f) { # for each iteration, the current file path is passed as the argument `f` to the anonymous function! so it will do this for each daily raster within each year
    ...
    # out_file <- file.path(daily_int_dir, paste0("daily_proj_crop_sst_", date, ".tif"))
    # terra::writeRaster(r, filename = out_file, overwrite = TRUE)
  ...
  return(file.path(hist_int_dir, paste0("hist_sst_avg_", year, ".tif"))) # returns a single layer raster where each pixel is the average of the SST over all days in the year, saves in Mazu
}
...
hist_yearly_averages <- foreach(year = hist_start_year:hist_end_year) %dopar% {
  ...
  process_sst_daily(year, goc_ecoregion_border_shp)
  
}
toc()

# bring all yearly averages into a single multilayer raster
hist_final_raster <- terra::rast(hist_yearly_averages)

# save it to int_dir in mazu: "/home/shares/ohi/OHI_GOC/_raw_data/CEDA_CDRv3.0/d2025/int"
writeRaster(hist_final_raster, filename = here(int_dir, "historical_yearly_sst_averages_goc_eq_area_1980_1990.tif"), overwrite = TRUE)
```
