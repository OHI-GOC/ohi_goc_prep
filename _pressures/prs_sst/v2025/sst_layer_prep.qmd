...
process_year <- function(year, goc_ecoregion_border_shp) {
  ...
  daily_rasters <- lapply(files, function(f) { # for each iteration, the current file path is passed as the argument `f` to the anonymous function! so it will do this for each daily raster within each year
    ...
    r <- terra::project(r, crs(goc_ecoregion_border_shp)) # NAD93, albers eq area
    ...
    r <- terra::crop(r, goc_ecoregion_border_shp) # makes the maximum extent of the daily raster limited to the goc ecoregion boundary polygon
  ...
  return(yearly_avg) # returns a single layer raster where each pixel is the average of the SST over all days in the year
}
...
# use the function
yearly_averages <- lapply(start_year:end_year, function(year) process_year(year, goc_ecoregion_border_shp))

# bring all yearly averages into a single multilayer raster
final_raster <- rast(yearly_averages)

# save it to output_dir
writeRaster(final_raster, filename = here(output_dir, "yearly_sst_averages_goc.tif"), overwrite = TRUE)
