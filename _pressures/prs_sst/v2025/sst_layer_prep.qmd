...
process_sst_daily <- function(year, goc_ecoregion_border_shp) {
  ...
  daily_rasters <- lapply(files, function(f) { # for each iteration, the current file path is passed as the argument `f` to the anonymous function! so it will do this for each daily raster within each year
    ...
    r <- terra::crop(r, goc_ecoregion_border_sst_proj, mask = TRUE) # makes the maximum extent of the daily raster limited to the goc ecoregion boundary polygon
...
```
Plot the yearly average to understand it better

```{r}
year_avg_1980 <- st_read(here(hist_int_dir, "hist_sst_yearly_avg_1980.tif"))
