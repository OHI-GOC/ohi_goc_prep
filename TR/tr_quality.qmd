...
```{r}
sesnsp_goc <- st_read(here::here("SS/v2025/int_sesnsp","sesnsp_goc_municipios_50km.shp")) ## need to fix the names / make a document describing the abbreviations!!

unique(sesnsp_goc$rgn_id) # [1] "Region_1" "Region_3" "Region_4" "Region_5" "Region_7" "Region_8" "Region_9"
```
...
tr_qual_crime_score <- sesnsp_goc_tr_qual_sum %>%
  group_by(ano) %>% 
  mutate(
    max_crime = max(number_crimes, na.rm = TRUE),
    current_status = (1 - (number_crimes / max_crime)) * 100
  ) %>%
  ungroup()
```
