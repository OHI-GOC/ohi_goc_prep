...
The new v4 API has been introduced and the necessary revisions have been made. The introduction of the new API made it difficult to tie subpopulations to specific regions, as there is no longer a way to query a list of countries for a species or subpopulation (see issue #351). However, there also may no longer be assessments associated with subpopulations. I emailed the IUCN to inquire further. The current methods don't include subpopulations in the final outputs, but the data didn't dramatically change between 2024 and 2025, so we went ahead with the score check. 
...
**List of iconic species:** Original List created in initial 2012 OHI assessment. Additional marine species were added from [Reyes-García et al. 2023](https://www.pnas.org/doi/10.1073/pnas.2217303120) in v2023.
...
For each of these species, use the IUCN API to gather a list of countries in which it is present. We will have to do this by querying for all the assessments for all countries and then filtering by species of interest.

**This takes a long time (and failed multiple times) so get started early or run overnight!**
...
ico_spp_rgn_prepped_v2024 <- here::here(goal, previous_scen, 'int', 'ico_spp_rgn_prepped.csv') %>%
  readr::read_csv() %>%
  dplyr::mutate(year = 2024)

# only one duplicate species; must be something else causing the issue

jordan1 <- ico_spp_rgn_prepped_v2024 %>% 
