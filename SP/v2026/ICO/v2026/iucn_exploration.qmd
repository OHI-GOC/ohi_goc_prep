...
[REFERENCE RMD
FILE](http://ohi-science.org/ohiprep_v2022/globalprep/ico/v2022/ico_data_prep.Rmd)

# Summary: Iconic Species Subgoal (Sense of Place)

This script prepares scores (status and trend) for Iconic Species in
each global coastal region. For each iconic marine species, the
countries of occurrence are pulled from the IUCN API. Extinction risk
categories for each species are pulled based on current and past
assessments; by tracking the assessed extinction risk over time, we can
understand the trends of extinction risk for iconic species directly
rather than using the "population trend" method from prior OHI
assessments.

The Iconic Species sub-goal model calculates a region's status based
upon an unweighted average of species health for all 'iconic' species
found within each reporting region.

From Halpern et al (2012):

> Iconic species are those that are relevant to local cultural identity
> through a species' relationship to one or more of the following: 1)
> traditional activities such as fishing, hunting or commerce; 2) local
> ethnic or religious practices; 3) existence value; and 4)
> locally-recognized aesthetic value (e.g., touristic attractions/common
> subjects for art such as whales). Habitat-forming species are not
> included in this definition of iconic species, nor are species that
> are harvested solely for economic or utilitarian purposes (even though
> they may be iconic to a sector or individual).

> Ultimately, almost any species can be iconic to someone, and so the
> intent with this goal was to focus on those species widely seen as
> iconic within a country, and iconic from a cultural or existence value
> (rather than for a livelihoods or extractive reason).

> The reference point is to have the risk status of all assessed species
> as Least Concern (i.e., a goal score = 1.0)

The Status of this sub-goal (X~ICO~) is then the % of iconic species in
each threat category (as defined by the IUCN Red List), such that:
...
-   *w~cat~* is the status weight assigned for that category (note,
    these are the inverse of the risk value used in the SPP
    calculations):
-   'LC' = 1.0, 'NT' = 0.8, 'VU' = 0.6, 'EN' = 0.4, 'CR' = 0.2, 'EX' =
    0.0

ICO trend is calculated in a similar manner, but weightings are assigned
according to IUCN population trend: 'Decreasing' = -0.5, 'Stable' = 0.0,
'Increasing' = +0.5.

# v2025 updates:

The new v4 API has been introduced and the necessary revisions have been
made. The introduction of the new API made it difficult to tie
subpopulations to specific regions, as there is no longer a way to query
a list of countries for a species or subpopulation (see issue #351).
However, there also may no longer be assessments associated with
subpopulations. I emailed the IUCN to inquire further. The current
methods don't include subpopulations in the final outputs, but the data
didn't dramatically change between 2024 and 2025, so we went ahead with
the score check.

# v2024 updates:

Additional year of IUCN Red List data added. Note that the Red List v3
API will be deprecated soon, in lieu of the v4 API.

# v2023 updates:

Additional year of IUCN Red List data added.

The iconic species list was updated by adding species from [Reyes-García
et al. 2023](https://www.pnas.org/doi/10.1073/pnas.2217303120).

# Data Sources

**List of iconic species:** Original List created in initial 2012 OHI
assessment. Additional marine species were added from [Reyes-García et
al. 2023](https://www.pnas.org/doi/10.1073/pnas.2217303120) in v2023.

-   **Reference** Reyes-García, Victoria, Rodrigo Cámara-Leret,
    Benjamin S. Halpern, Casey O’Hara, Delphine Renard, Noelia
    Zafra-Calvo, and Sandra Díaz. 2023. “Biocultural Vulnerability
    Exposes Threats of Culturally Important Species.” Proceedings of the
    National Academy of Sciences 120 (2): e2217303120.
    <https://doi.org/10.1073/pnas.2217303120>.

**Species native country and status information:**

-   **Reference**: IUCN 2024. IUCN Red List of Threatened Species.
    Version 2025-1 \<www.iucnredlist.org\>
...
goal <- here::here("globalprep","ico")
dir_data <- here(dir_M, "git-annex", "globalprep","ico")
dir_here <- here::here(goal, version_year)

source(here::here(dir_here, 'ico_fxn.R'))
```

Using the IUCN API, we accessed the full IUCN species list. With some
minor formatting, this list contains the following variables:

iucn_sid \| kingdom \| phylum \| class \| order \| family \| genus \|
sciname \| subpopulation \| category

## Download Species List from IUCN

Get total species list, using the IUCN API.

To access the API v4, you will need to sign up for a key on the [IUCN
website](https://api.iucnredlist.org/). This key can be added to the
local .renviron on your machine and retrieved using the code below.
...
The list of Iconic Species is based upon the original ICO list generated
in 2011, using species identified as globally iconic (WWF Flagship
species and Priority species) or regionally iconic (based upon WWF
regional/local priority species and nation-specific lists).

2023 update: The iconic species list was updated by adding species from
[Reyes-García et al.
2023](https://www.pnas.org/doi/10.1073/pnas.2217303120). The list should
be read directly from the raw data folder. This file will be copied from
year to year unless further updates are made to the list.
...
ico_list_raw <- read_csv(here("globalprep","ico", version_year, "raw","ico_list_raw_updated.csv"), show_col_types = FALSE)
```
...
# Query taxa info for iconic species
for (i in unique(ico_list_raw$sciname)) {
  
  #i <- "Dugong dugon"
  
  spp <- unlist(str_split(i, " "))
  
  results <- rl_species(spp[1], spp[2])
  
  iucn_sid <- results$taxon$sis_id
  sciname <- results$taxon$scientific_name
  cat <- results$assessments %>%
    filter(latest == TRUE) %>%
    unnest(scopes, names_sep = "_") %>%
    filter(scopes_description$en == "Global") %>%
    pull(red_list_category_code)
  
  # Handle subpopulations
  subpop <- results$taxon$subpopulation_taxa
  subpop_df <- NULL

  if (!is.null(subpop) && is.data.frame(subpop) && nrow(subpop) > 0) {
    subpop_df <- subpop %>%
      rowwise() %>%
      mutate(
        cat = tryCatch(
          rl_sis(sis_id)$assessments %>%
            filter(latest == TRUE) %>%
            pull(red_list_category_code) %>%
            .[1],
          error = function(e) NA
      ...
      ungroup() %>%
      select(iucn_sid = sis_id, sciname = scientific_name, subpop = subpopulation_name, cat)
  }
  ...
  # Combine species info and subpopulations (if any)
  spp_info <- bind_rows(main_row, subpop_df)
  
  # Bind new df to existing df
  spp_ico <- bind_rows(spp_ico, spp_info)
...
# Save for time
write_csv(spp_ico, file.path(dir_here, sprintf('raw/spp_list_from_api_ico_only.csv')))
```
...
spp_ico <- read_csv(here::here(dir_here, 'raw', 'spp_list_from_api_ico_only.csv'), show_col_types = FALSE)
...
nrow(spp_missing) 
# v2025: 1 (Maui's dolphin)
# v2024: 0

# Add missing species
if (nrow(spp_missing) > 0) {
  
  # Read in old data
  prev_year <- read_csv(file.path('/home/hoyt/OHI/ohiprep_v2025/globalprep/ico/v2024/int/ico_list_prepped.csv'), show_col_types = FALSE)
  
  # Find IUCN ID of missing species
  missing <- prev_year %>%
    filter(sciname %in% unique(spp_missing$sciname)) %>%
    pull(iucn_sid)
  
  # Query for missing species
  results <- rl_sis(id = missing)
 ...
 iucn_sid <- results$taxon$sis_id
  ...
  # Combine species row and subpopulations (if any)
  spp_info <- bind_rows(main_row, subpop_df)
...
ico_list <- ico_list_raw %>%
  dplyr::left_join(spp_ico %>% 
                     dplyr::select(iucn_sid, sciname, subpop, cat),
...
readr::write_csv(ico_list, here::here(dir_here, 'int', 'ico_list_prepped.csv')) 
# 270 in v2021, 275 in v2022, 
#5698 in 2023 (increase due to new species being region specific rather than global)

# v2024: 7612 observations, 127 unique species by sciname.
# v2025: 7518 observations, 127 unique species by sciname
```
...
For each of these species, use the IUCN API to gather a list of
countries in which it is present. We will have to do this by querying
for all the assessments for all countries and then filtering by species
of interest.

**This takes a long time (and failed multiple times) so get started
early or run overnight!**

```{r get_country_list}

# Get species of interest
ico_list <- read_csv(file.path(dir_here, 'int', 'ico_list_prepped.csv'), show_col_types = FALSE)
spp_ids <- unique(ico_list$iucn_sid)

# Get df of countries
countries <- rl_countries()$countries
countries$description <- countries$description$en

# Generate empty list
country_assessments <- list()

start_time <- Sys.time()

for (i in countries$code[1:76]) {
  
  country <- countries$description[countries$code == i]
  
  assessments <- rl_countries(i)$assessments
  
  country_assessments[[country]] <- dplyr::as_tibble(assessments)
...
country_assessments_complete <- bind_rows(country_assessments) %>%
    filter(latest == TRUE) %>%
    unnest(scopes, name_sep = "_") %>%
    left_join(countries, by = "code") 

country_assessments_complete$description <- country_assessments_complete$description$en

# Save for time
write_csv(country_assessments_complete, file.path(dir_data, 'v2025', 'iucn_countries_api.csv'))

# Load for time
country_assessments <- read_csv(file.path(dir_data, 'v2025', 'iucn_countries_api.csv'), show_col_types = FALSE)

# Check to make sure all countries were queried
setdiff(countries$code, unique(country_assessments_complete$code))

country_assessments_filtered <- country_assessments_complete %>%
  filter(latest == TRUE) %>%
  #unnest(scopes, names_sep = "_") %>%
  filter(scopes_description == "Global") %>%
  left_join(countries, by = "code") %>%
  select(iucn_sid = sis_taxon_id, 
         year = year_published, 
         possibly_extinct, 
         possibly_extinct_in_the_wild, 
         sciname = taxon_scientific_name, 
         cat = red_list_category_code, 
         code, 
         country = description) %>%
  distinct()

spp_countries_df <- country_assessments_filtered %>%
  filter(iucn_sid %in% spp_ids) %>%
  group_by(country) %>%
  mutate(count = n(),
         # Using the `possibly_extinct` column instead of the `possibly_extinct_in_the_wild`
         presence = ifelse(possibly_extinct == TRUE, "Possibly Extinct", "Extant")) %>%
  select(iucn_sid, count, code, country, presence)
...
Name to region function (in OHI core package) reports regions that don't
have a match in OHI region list. Here we report certain reported regions
at a higher spatial scale, based on the listed regions in the error
message.

```{r country_split}
# bring back in the reported regions associated with the iucn ids
ico_spp_countries <- read_csv(here::here(dir_here, 'int', 'ico_spp_countries.csv'), show_col_types = FALSE)
...
# v2025 - ALSO ALSO no match for Isle of Man (not reported in OHI), Saint Barthélemy (not reported in OHI), disputed territory, can't be matched to a country
...
check_full <- ico_spp_countries %>% 
  dplyr::filter(country == "Bonaire, Sint Eustatius and Saba") # v2025, v2024, and v2023: 34 rows

check_split <- ico_spp_rgn_raw %>% 
  dplyr::filter(rgn_name == "Bonaire" | rgn_name == "Saba" | rgn_name == "Sint Eustatius") # v2025, v2024, and v2023: 102 rows

# 102 entries makes sense each of the 34 rows is listed three times, one for each country

# ---- French Southern Territories ----
check_full <- ico_spp_countries %>%
  dplyr::filter(country == "French Southern Territories") # v2025: 26, v2024: 25

check_split <- ico_spp_rgn_raw %>% 
  dplyr::filter(rgn_name %in% c("Glorioso Islands", "Juan de Nova Island", "Bassas da India", "Ile Europa", "Ile Tromelin", "Crozet Islands", "Amsterdam Island and Saint Paul Island", "Kerguelen Islands")) 
# v2025: 208
# 8(regions) * 26(rows) = 208,
#v2024: 200
# 25(rows) * 8(regions) = 200

# ---- United States Minor Outlying Islands ----
check_full <- ico_spp_countries %>%
  dplyr::filter(country == "United States Minor Outlying Islands") # v2025: 21, v2024: 18

check_split <- ico_spp_rgn_raw  %>% 
  dplyr::filter(rgn_name %in% c("Wake Island", "Jarvis Island", "Palmyra Atoll", "Howland Island and Baker Island", "Johnston Atoll")) 
# v2025: 105
# 21 rows * 5 regions = 105
# v2024: 90
# 18 rows * 5 regions = 90

# ---- Saint Helena, Ascension and Tristan da Cunha ----
check_full <- ico_spp_countries %>% 
  dplyr::filter(country == "Saint Helena, Ascension and Tristan da Cunha") #v2025: 34, v2024: 33 rows

check_split <- ico_spp_rgn_raw %>% 
  dplyr::filter(rgn_name == "Saint Helena" | rgn_name == "Ascension" | rgn_name == "Tristan da Cunha") 
# v2025: 102 rows
# v2024: 99 rows
...
ico_spp_rgn_raw <- ico_spp_rgn_raw %>% 
  #select(-c(distribution_code, origin, code, country)) %>% 
  distinct()
...
ico_spp_rgn_raw <- read_csv(here::here(dir_here, 'raw', 'ico_spp_rgn_raw.csv'), show_col_types = FALSE)

ico_list <- read_csv(here::here(dir_here, 'int', 'ico_list_prepped.csv'), show_col_types = FALSE)
...
# Adding as.tibble() simplified the structure, prevents ico_rgn_id from being coerced to logical structure
# v2024: 50,734 obs --> 3857 obs
# v2025: 45,235 obs --> 3424 obs
...
ico_spp_rgn_prepped <- read_csv(here::here(goal, version_year, 'int', 'ico_spp_rgn_prepped.csv'), col_types = "ddcccdcccldcc")
...
We accessed the IUCN API to determine past IUCN assessments for each of
the identified iconic species:
<http://apiv3.iucnredlist.org/api/v3/species/history/id/><species id>?token=<token>.

Each assessment includes a year and an extinction risk, along with
...
# Initiate data frame
spp_assessments <- data.frame()

# Save categories
categories <- rl_categories()$red_list_categories

categories$description <- categories$description$en
...
assessment <- result$assessments %>%
  unnest(scopes, names_sep = "_") %>%
  filter(scopes_description$en == "Global") %>%
  select(iucn_sid = sis_taxon_id, year = year_published, code = red_list_category_code, sciname = taxon_scientific_name, latest) %>%
  left_join(categories, by = "code", relationship = "many-to-many") %>%
  group_by(iucn_sid) %>%
  mutate(assess_year = year[latest == TRUE][1]) %>%
  ungroup() %>%
  select(iucn_sid, year, assess_year, code, category = description, sciname) %>%
  distinct()

spp_assessments <- bind_rows(spp_assessments, assessment)

Sys.sleep(2)

}

ico_assess_raw <- spp_assessments

# 705 in v2018, 743 in v2019, 463 in v2020, 480 in v2021, 496 in v2022, #783 in v2023, #828 in v2024, #530 in v2025

# ico_assess_raw <- read_csv(file.path(dir_data, 'v2025', 'iucn_countries_api.csv'), show_col_types = FALSE) %>%
#   filter(sis_taxon_id %in% spp_ids) %>%
#   left_join(cat_lookup, by = c("red_list_category_code" = "code")) %>%
#   group_by(sis_taxon_id) %>%
#   mutate(assess_year = year_published[latest == TRUE][1]) %>%
#   ungroup() %>%
#   select(iucn_sid = sis_taxon_id, year = year_published, assess_year, code = code_current, category, sciname = taxon_scientific_name) %>%
#   distinct()
# 
...
# v2025: 135631 was off by +2, everything else (20 other species) was +/-1
...
# v2025: no NA, but only 1 NA for 170759990 in 2024?
...
These raw assessments are cleaned up to standardize codes and
categories, and category scores are assigned based on a scale from Least
Concern = 1 to Extinct = 0, as per the [Species OHI
subgoal](https://rawgit.com/OHI-Science/ohi-global/draft/global_supplement/Supplement.html#591_iconic_species_(subgoal_of_sense_of_place)).
Note that past assessments used different coding, especially prior to
1994; we reclassified older codes according to this chart:
...
ico_assess_raw <- read_csv(here::here(dir_here, 'raw', 'ico_assessments_raw.csv'), show_col_types = FALSE)
...
ico_assess <- ico_assess_raw %>% 
  dplyr::left_join(pop_cat, by = c("code" = "cat")) %>%
...
Using `tidyr::complete()` and `tidyr::fill()`, we create a full time
series for all species from the earliest assessment to the most recent
year.

```{r fill_out_category_time_series}

ico_assess <- read_csv(here::here(dir_here, 'int', 'ico_assess_clean.csv'), show_col_types = FALSE)
ico_list <- read_csv(here::here(dir_here, 'int', 'ico_list_prepped.csv'), show_col_types = FALSE)
...
ico_spp_cat <- ico_spp_cat %>%
  dplyr::left_join(pop_cat %>% 
                     dplyr::rename(cat_2024 = cat, cat_2024_score = cat_score), 
                   by = 'cat_2024') %>%
...
summary(ico_spp_cat)
# v2025: 2365-38493-2451 NAs for cat_score, eval_yr, cat_2024_score
...
Using `dplyr::full_join()` we combine the `ico_spp_rgn` dataframe
(iconic species by OHI region) with the `ico_spp_cat` dataframe (iconic
species by category and year, with species info, year, and category
info).
...
The toolbox wants `rgn_id`, species `sciname`, and extinction risk
`category` for the basic calculations. Since some regions contain
multiple subpops (or parent/subpop) we also include `iucn_sid` to
differentiate. This information is included for each `year`, filtered
back to the year 2000.

While the official calculations are performed in the toolbox, we perform
the same basic calcs here to get a sense of the ICO status and trend
ahead of time. Report and summarize estimate of regional iconic species
status:
...
# v2025:
 #   CR    EN    LC    NT    VU 
 # 4715 24450 22087 22260 36873 .... hmmm?
...
To examine results of the new methods (including API-based data scraping
and trend based on time series), we plot the estimated status and trend
over time.
...
## Compare OHI 2024 vs OHI 2023 assessment results

Plot the estimated status and trends from assessments v2023 and v2024 to
compare.
...
v2021 *No need to run this section of the code; for data exploration
purposes only*.

Let's check rgn id 215.

\`\`\`{r outliers, include=FALSE} ico_spp_rgn_prepped \<-
here::here(goal, version_year, 'int', 'ico_spp_rgn_prepped.csv') %\>%
readr::read_csv(col_types = "ddcccccdcccldcc") %\>% dplyr::mutate(year =
2025)

# Read in ico_spp_countries from v2018

ico_spp_rgn_prepped_v2024 \<- here::here(goal, previous_scen, 'int',
'ico_spp_rgn_prepped.csv') %\>% readr::read_csv() %\>%
dplyr::mutate(year = 2024)

# only one duplicate species; must be something else causing the issue

jordan1 \<- ico_spp_rgn_prepped_v2024 %\>% dplyr::filter(rgn_name ==
"Jordan") %\>% dplyr::select(iucn_sid, presence, rgn_id, rgn_name,
comname, sciname, cat, year)

jordan2 \<- ico_spp_rgn_prepped %\>% dplyr::filter(rgn_name == "Jordan")
%\>% dplyr::select(iucn_sid, presence, rgn_id, rgn_name, comname,
sciname, cat, year)

jordan \<- rbind(jordan1, jordan2)

qatar1 \<- ico_spp_rgn_prepped_v2020 %\>% dplyr::filter(rgn_name ==
"Qatar") %\>% dplyr::select(iucn_sid, presence, rgn_id, rgn_name,
comname, sciname, cat, year)

qatar2 \<- ico_spp_rgn_prepped %\>% dplyr::filter(rgn_name == "Qatar")
%\>% dplyr::select(iucn_sid, presence, rgn_id, rgn_name, comname,
sciname, cat, year)
