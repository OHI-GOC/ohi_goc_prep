---
title: 'OHI GoC 2025: Clean Waters: Secchi Depth'
...
# Global Ocean Colour (Copernicus-GlobColour), Bio-Geo-Chemical, L4 (monthly and interpolated) from Satellite Observations (1997-ongoing): Secchi depth of sea water
...
*Dataset*
Transparency, multi-sensor (4 km), monthly   ·   cmems_obs-oc_glo_bgc-transp_my_l4-multi-4km_P1M

<https://data.marine.copernicus.eu/product/OCEANCOLOUR_GLO_BGC_L4_MY_009_104/download?dataset=cmems_obs-oc_glo_bgc-transp_my_l4-gapfree-multi-4km_P1D_202311> 
...
int_monthly_dir <- here("/home/shares/ohi/OHI_GOC/goal_prep/cw/v2025/secchi_int/monthly_rasters")
# dir.create(int_monthly_dir)
...
4.  This will bring you to their graphical tool download page. For the [**Variables**]{.underline}, "Clear all" and select "Secchi depth of sea waterZSD [m]". Scroll to the bottom of the page, and under **Date Range**, specify 01/09/1997, 00:00 to 01/09/2025, 00:00.
...
copernicusmarine.login(username='smanos', password='OHI_GOC_copernicus_2025') 
...
copernicusmarine.subset( # start of copied code from CMS website
  ...
  variables=["ZSD"],
  minimum_longitude=-119.73200982142856,
  maximum_longitude=-99.24096116071428,
  minimum_latitude=16.74942335379463,
  maximum_latitude=34.026189871651766,
  start_datetime="1997-09-01T00:00:00",
  end_datetime="2025-09-01T00:00:00", # end of copied code from CMS website
  
  output_filename=os.path.join(cms_dir, "global_ocean_color_secchi.nc") ## This line is not copied!!! Need it to specify the Mazu directory for the data to download to
  
  ) # v2025 python API download code, which will download as a singular multi-layer NetCDF file (nc-file)

print(f"Data downloaded to: {os.path.join(cms_dir, 'global_ocean_color_secchi.nc')}") # print in console: data download finished! Data downloaded to: /home/shares/ohi/OHI_GOC/_raw_data/CMS/d2025/global_ocean_color_secchi.nc
```
...
secchi_raster <- terra::rast(here::here(cms_dir, "global_ocean_color_secchi.nc")) # Warning: [rast] unknown calendar (assuming standard): Gregorian

# check the current CRS
crs(secchi_raster) # v2025: ""GEOGCRS[\"WGS 84 (CRS84)\",\n".  
plot(secchi_raster) # to get a quick visualization of the multi-layer SpatRaster

# since nc_raster is already loaded as a SpatRaster object
# check the number of layers
num_layers <- terra::nlyr(secchi_raster)
...
time_attribute <- terra::time(secchi_raster)
...
for (i in 1:num_layers) {
  layer <- secchi_raster[[i]] # the layer in the seq from 1:337 (v2025)
...
plot(rast_2001_10_01, main = "Secchi Depth of Sea Water (m) - October 2001", xlab = "Longitude", ylab = "Latitude") # plot it!
