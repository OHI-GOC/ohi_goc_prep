...
tidy_wdpa_data <- function(wdpa_poly_object) {
  ...
  DF <- wdpa_poly_object %>%
    ...
    dplyr::select(wdpaid, name, orig_name, desig, desig_eng, desig_type, iucn_cat, 
...
```{r}
eco_hr_latlon <- st_read(here(goc_spatial, "OpenStreetMapCoastline", "GoC_polygon_ecoregion_latlon.shp"))
# mapview(eco_hr_latlon)

eco_hr_eqarea <- st_read(here(goc_spatial, "OpenStreetMapCoastline", "GoC_polygon_ecoregion_eqarea.shp"))
...
I want to make sure that land protected areas are incorporated as well.
...
```{r}
hem_protected_anps <- st_read(here::here("/home/shares/ohi/OHI_GOC/spatial/from_hem_POEM_ANP/232_ANP-ITRF08_16062025","232_ANP-ITRF08_16062025.shp")) %>%
  filter(estados %in% c("Baja California Sur","Jalisco","Baja California","Sonora","Nayarit","Sinaloa","Baja California y Baja California Sur","B.C., B.C.S., Sonora y Sinaloa","Jalisco y Nayarit")) %>% 
  janitor::clean_names()

mapview(hem_protected_anps)

loreto_hem_anps <- hem_protected_anps %>% 
  filter(nombre %in% c("Bahía de Loreto","Loreto II"))
mapview(loreto_hem_anps, zcol = "nombre")
```

```{r}
unique(hem_anps_land$estados)

hem_anps_land <- hem_protected_anps %>% 
  filter(s_terres != 0) %>% 
  filter(estados %in% c("Baja California Sur","Jalisco","Baja California","Sonora","Nayarit","Sinaloa","Baja California y Baja California Sur","B.C., B.C.S., Sonora y Sinaloa","Jalisco y Nayarit")) %>%  # found by running `unique(hem_anps_land$estados)`
  filter(!nombre %in% "Islas del Golfo de California")

hem_anps_islands <- hem_protected_anps %>% 
  filter(nombre %in% "Islas del Golfo de California")

mapview(hem_anps_land, col.region = "black") + mapview(wdpa_poly_all, col.region = "blue") + mapview(hem_anps_islands, col.region = "darkgreen")
```

```{r}
lsp_list_ewg_v1 <- read_csv(here::here("/home/shares/ohi/OHI_GOC/goal_prep/sp/lsp/v2025/LSP_activity_estimated_coordinates_v1_041526.csv")) %>% 
  janitor::clean_names()

lsp_est_lat_longs_int <- lsp_list_ewg_v1 %>% 
  mutate(long = as.numeric(long),
         lat = as.numeric(lat)) 

summary(lsp_est_lat_longs_int) # 41 NAs for now.... out of 130 LSPs.

lsp_est_lat_longs <- lsp_est_lat_longs_int %>% 
  filter(!is.na(long)) %>% 
  st_as_sf(
...
mapview(hem_anps_land, col.region = "black") + mapview(wdpa_poly_all, col.region = "blue") + mapview(hem_anps_islands, col.region = "darkgreen") + mapview(lsp_est_lat_longs, col.region = "orange")
```

```{r}
# how many of the NAs are not lasting special places at all?
lsp_comments <- lsp_list_ewg_v1 %>% 
  filter(is.na(how_to_download))

length(lsp_comments) # 9 
```


## Evaluating the 2020 GCMP CBMC project shapefiles available
...
mapview(final_mpa_layer) + mapview(lsp_summary, zcol = "in_pa_poly")
```

```{r}
dive_sites <- st_read(here::here("/home/shares/ohi/OHI_GOC/_raw_data/CBMC-GCMP-prioritization-of-diving-sites-main/shp/Diving_sites_v1.0_01032021/dive_sites.shp")) %>% 
  janitor::clean_names()
...
```{r}
mapview(dive_sites, zcol = "protection") + mapview(hem_anps_land, col.region = "black") + mapview(wdpa_poly_all, col.region = "blue") + mapview(hem_anps_islands, col.region = "darkgreen") + mapview(lsp_est_lat_longs, col.region = "orange")
```

```{r}
i_efectividad_pp <- read_csv(here::here("/home/shares/ohi/OHI_GOC/_raw_data/SIMEC/protectedplanet-effectiveness-Apr2026.csv"))
# unfortunately, the protected planet effectiveness is not accurate because when I compare the actual SIMEC reports to the protected planet data, there are inconsistencies. For example, in 2021 Bahia de Loreto had "Índice de Efectividad 92 Efectividad sobresaliente" here: <https://simec.conanp.gob.mx/i-efectividad_docs/PBCyPN/PN%20Bah%C3%ADa%20de%20Loreto.pdf> but the protected planet data does not have the assessment year or submitted year for 2021 (only 2018 or 2022) and for those two years, the score was 68. Which is very unlikely considering Bahia de Loreto has such a high score on the i-efectividad report card, as well as it is on the IUCN Green List of Protected and Conserved Areas <https://simec.conanp.gob.mx/lineamientosLV.php#5/28.883/-110.171>
```
